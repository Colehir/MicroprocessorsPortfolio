...
// beginning of area for all students changes
		...
		else
		{
			...
			if(temp >= temproom * 1.03) {P1OUT |=  BIT5; __delay_cycles(200);}
			if(temp <= temproom * 1.01) {P1OUT &= ~BIT5; __delay_cycles(200);}
		}
		...
		else
		{
			...
			if(touch >= touchroom * 0.9) {__delay_cycles(200);changed = 0;}  // off
			if((touch <= touchroom * 0.7)&&!changed) {P2OUT ^=  BIT0; __delay_cycles(200);changed=1;}  // on
		}
