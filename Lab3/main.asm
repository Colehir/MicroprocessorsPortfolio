...
Summation	mov.w 	R5, R6		;starting the upper limit with value a of the summation in R6
			...
			clrc
			rlc 	R6;finish Xcalcby dobling the value and adding it to accumulator R8,
			add.w	R6, R8;repeat the loop with a-1, a-2 (in R5) till lower value a=0
			dec 	R5
			jne		Summation
			incd	R8;0 Factorial = 1
SumEnd		mov.w	R8,R5;save the Xcalc results in R5
...
skip1		tst		R6
			...
			inc		R6
			inc		R9
skip2		mov.w	#8, R8		;8 bit multiplication, so we loop 8 times
